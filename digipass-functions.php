...
function get_images($attr)
{
    ...
    foreach ($attachments as $attachment) {
        ...
        if (!empty($imgSrc)) {
            ...
            $item['ident_nr'] = wptexturize(get_post_meta($attachment->ID, 'digipass-ident_nr', true));
            $item['source'] = wptexturize(get_post_meta($attachment->ID, 'digipass-source', true));
            $item['author'] = wptexturize(get_post_meta($attachment->ID, 'digipass-author', true));
            $item['publisher'] = wptexturize(get_post_meta($attachment->ID, 'digipass-publisher', true));
            $item['license'] = wptexturize(get_post_meta($attachment->ID, 'digipass-license', true));
            $item['link'] = wptexturize(get_post_meta($attachment->ID, 'digipass-link', true));
