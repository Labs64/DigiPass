...
// add shortcodes
add_shortcode('digipass_table', 'digipass_table_shortcode');
add_filter('img_caption_shortcode', 'digipass_caption_shortcode_filter', 10, 3);

function digipass_table_shortcode($atts)
{
    ...
    $ret = '<table id="digipass-table" class="digipass-' . $style . '"><thead>';
    $ret .= '<th>' . '&nbsp;' . '</th>';
    ...
    if (empty($images)) {
        $ret .= '<tr class="digipass-row"><td colspan="6" class="digipass-column-empty">' . __('No images found', CT_SLUG) . '</td></tr>';
    }
...
function digipass_caption_shortcode_filter($val, $attr, $content = null)
{
    extract(shortcode_atts(
            array(
                'id' => '',
